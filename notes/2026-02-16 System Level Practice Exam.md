...
Ans: fork(), open(), write(), read(), exec()

### What is an application programming interface (API)?
Ans: An API is a collection of libraries usually provided by the host operating system so that application programmers can use them to communicate with the Operating System for any lower-level (usually hardware-level) tasks.
...
### Supply parenthesis so that it specifies how C compiler will evaluate
